...
namespace KanBankasi
{
    public partial class HastaListesi : Form
    {
        ...
        {
            baglanti.Open();
            string query = "select * from HastaTbl"; // Hasta tablosundan çekiyoruz
        ...
        // Arama Kutusu (Bunu Events kısmından TextChanged'e bağlamayı unutma!)
        private void txtAra_TextChanged(object sender, EventArgs e)
        {
            baglanti.Open();
            // HAdSoyad sütununda arama yapıyoruz
            string query = "select * from HastaTbl where HAdSoyad like '%" + txtAra.Text + "%'";
            SqlDataAdapter sda = new SqlDataAdapter(query, baglanti);
            var ds = new DataSet();
            sda.Fill(ds);
            HastaDGV.DataSource = ds.Tables[0];
